...
export type JobPostPageInterviewData = Pick<InterviewExperienceTable, "id" | "round_no" | "difficulty" | "description" | "interview_date" | "response_date" | "interview_tags" | "created_at"> & {
  [DBTable.APPLICATION]: Pick<ApplicationTable, "id" | "job_posting_id" | "status">;
  [DBTable.USER]: Pick<User, "full_name" | "profile_pic_url">;
};

export function InterviewExperienceCard({ interviewExperience }: { interviewExperience: JobPostPageInterviewData }) {
  console.log("this interview experience", interviewExperience);

  const router = useRouter();

  const handleCardClick = () => {
    router.push(`/interview/${interviewExperience[DBTable.APPLICATION].id}`);
  };

  return (
    <motion.div transition={{ type: "spring", stiffness: 300 }} whileHover={{ scale: 1.02 }}>
      <Card isPressable className="dark:bg-content1-dark w-full border border-gray-200 bg-content1 dark:border-gray-700" onPress={handleCardClick}>
