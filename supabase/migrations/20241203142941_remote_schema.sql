drop function if exists "public"."update_job_with_countries"(p_job_posting_id uuid, p_title text, p_url text, p_country_ids uuid[], p_closed_date date, p_job_status text, p_job_posted_date date, p_experience_level_ids uuid[]);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.update_job_with_countries(p_job_posting_id uuid, p_title text, p_url text, p_country_ids uuid[], p_closed_date date, p_job_status text, p_job_posted_date date, p_experience_level_ids uuid[], p_job_category_ids uuid[])
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
