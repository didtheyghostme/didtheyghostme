drop function if exists "public"."get_all_search_jobs"(p_page integer, p_search text, p_is_verified boolean, p_country_ids uuid[]);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.get_all_search_jobs(p_page integer, p_search text, p_is_verified boolean, p_country_ids uuid[], p_sort_order text DEFAULT 'DESC'::text)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
