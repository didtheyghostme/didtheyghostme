drop function if exists "public"."insert_job_with_countries"(p_title text, p_url text, p_company_id uuid, p_user_id text, p_country_ids uuid[], p_experience_level_ids uuid[], p_job_category_ids uuid[]);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.insert_job_with_countries(p_title text, p_url text, p_company_id uuid, p_user_id text, p_country_names text[], p_experience_level_names text[], p_job_category_names text[])
 RETURNS void
 LANGUAGE plpgsql
AS $function$
declare
